...
from evaluate import anno_func
import json
    ...
    for batch_idx, (inputs, loc_targets, cls_targets) in tqdm(enumerate(valloader)):
        ...
        for i in range(loc_preds.size()[0]):
            ...
            for i,box in enumerate(boxes):
                # TODO solve error: tensor() is not JSON serializable
                bbox={}
                bbox['xmin']=float(box[0])
                bbox['xmax']=float(box[2])
                bbox['ymin']=float(box[1])
                bbox['ymax']=float(box[3])
...
if __name__ == '__main__':
    ...
    parser.add_option('--backbone','--backbone',dest='backbone',default='resnet50',
    ...
    if mode=='valid':
        ...
        sm = anno_func.eval_annos(annos, results_annos1, iou=0.5,types=anno_func.type5,minscore=50,check_type=True)
        print sm['report']
        ...
        for fname in img_list:
            ...
            plt.imsave('./result/'+fname, new_img)
            # plt.imshow(new_img)
            # plt.show()
    else:
