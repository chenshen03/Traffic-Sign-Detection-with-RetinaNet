...
if args.resume:
    ...
    checkpoint = torch.load(os.path.join('ckpts', args.exp, '30_ckpt.pth'))
...
# input_data = torch.rand(cfg.batch_size, 3, cfg.width, cfg.height)
...
def train(epoch):
    ...
    for batch_idx, (inputs, loc_targets, cls_targets) in enumerate(trainloader):
        ...
        if num_pos==0:
            ...
            continue
        loss, loc_loss, cls_loss = criterion(loc_preds, loc_targets, cls_preds, cls_targets, pos,batch_idx%2==0)
        ...
        # TODO: solve error: invalid index of a 0-dim tensor
        try:
        ...
        if batch_idx%2==0:
            print('train_loss: %.3f | avg_loss: %.4f' % (loss_data, train_loss/(batch_idx+1)))

    writer.add_scalar("data/loc_loss", loc_losses/batch_idx, epoch)
    writer.add_scalar("data/cls_loss", cls_losses/batch_idx, epoch)
    writer.add_scalar("data/avg_loss", train_loss/batch_idx, epoch)

    writer.add_scalars("data/train_loss", {'loc_loss': loc_losses/batch_idx,
                                           'cls_loss': cls_losses/batch_idx,
...
def val(epoch):
    ...
    for batch_idx, (inputs, loc_targets, cls_targets) in enumerate(valloader):
        ...
        # TODO: invalid index of a 0-dim tensor
        try:
            loss_data0 = loss.data[0]
        except:
            loss_data0 = loss.item()
        val_loss += loss_data0
        if batch_idx%10==0:
            print('val_loss: %.4f | avg_loss: %.4f' % (loss_data0, val_loss / (batch_idx + 1)))
